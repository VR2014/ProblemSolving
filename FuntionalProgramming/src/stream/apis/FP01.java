...
public class FP01 {
	...
	public static void main(String[] args) {
		...
		printAllCources(List.of("Spring", "Spring Boot", "API", "Micro services", "AWS", "PCF", "Azure", "DOCKER",
				"Kuberneties"));
		printAllCourcesContainSpring(List.of("Spring", "Spring Boot", "API", "Micro services", "AWS", "PCF", "Azure",
				"DOCKER", "Kuberneties"));
		printAllCourcesAtLeastFourLetters(List.of("Spring", "Spring Boot", "API", "Micro services", "AWS", "PCF",
				"Azure", "DOCKER", "Kuberneties"));
		...
		printSecondLargestCourseName(List.of("Spring", "Spring Boot", "API", "Micro services", "AWS", "PCF", "Azure",
				"DOCKER", "Kuberneties"));
	}

	private static void printSecondLargestCourseName(List<String> courseName) {
		System.out.println("printNoOfcharInEachCourseName   :::  ");
		courseName.stream().distinct()
		.sorted((s1, s2) -> Integer.compare(s2.length(), s1.length())).skip(1).findFirst()
	...
	private static void printNoOfcharInEachCourseName(List<String> courseName) {
		System.out.println("printNoOfcharInEachCourseName   :::  ");
		courseName.stream().map(word -> word.length())
	...
	private static void printCubesEvenNumbersFromList(List<Integer> cubes) {
		...
		cubes.stream().filter(x -> x % 2 == 0).map(v -> v * v * v).forEach(System.out::println);
	}
	...
	private static void printSquaresEvenNumbersFromList(List<Integer> square) {
		...
		square.stream().filter(x -> x % 2 == 0).map(x -> (int) Math.pow(x, 2)).forEach(System.out::println);
	}
	...
	private static void printAllCourcesAtLeastFourLetters(List<String> validations) {
		...
		validations.stream().filter(name -> name.length() >= 4).forEach(System.out::println);
	}

	private static void printAllCourcesContainSpring(List<String> valid) {
		System.out.println("printAllCourcesContainSpring   :::  ");
		valid.stream().filter(value -> value.contains("Spring")).forEach(System.out::println);
		;
